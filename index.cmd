...
if not exist "%~1"                                    ( goto ERROR_FILENOTEXIST )
echo [INFO] %~1                                                              1>&2
echo [INFO] Done.                                                            1>&2
echo.                                                                        1>&2


echo [INFO] pre-backup to .bak, program will overwrite the original-file.  1>&2
copy /b /v /y  "%~sdp1%~nx1"  "%~sdp1%~nx1.bak"
set "EXIT_CODE=%ErrorLevel%"
if ["%ErrorLevel%"] NEQ ["0"]                         ( goto ERROR_NOCOPY   )
...
echo  "%NODE%" "%SCRIPT%" "%~sdp1%~nx1"    1>&2
echo.                              1>&2
echo [INFO] program output:        1>&2
echo ----------------------------- 1>&2
call  "%NODE%" "%SCRIPT%" "%~sdp1%~nx1"
set "EXIT_CODE=%ErrorLevel%"
echo ----------------------------- 1>&2
echo [INFO] Done.                  1>&2
...
:ERROR_FILENOTEXIST
  set "EXIT_CODE=444"
  echo [ERROR] argument file does not exist. 1>&2
  goto END

:ERROR_NOCOPY
  set "EXIT_CODE=555"
  echo [ERROR] could not copy to .bak [%~1]    1>&2
  goto END
